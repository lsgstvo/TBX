...
return Widget:extend(function(self)
  ...
  div({ class = "admin-stats" }, function()
    local cards = {
      ...
      { tostring(#(self.lancamentos or {})),     "Lançamentos" },
    }
  ...
  if self.agendadas and #self.agendadas > 0 then
    div({ class = "admin-section shadow-card agendadas-section" }, function()
      ...
      element("table", { class = "admin-table" }, function()
        thead(function() tr(function() th("Título"); th("Publicar em"); th("Ações") end) end)
        tbody(function()
      ...
      script(function()
        raw(string.format([[
          (function(){
            ...
            if(!chart||dados.length===0){if(chart)chart.innerHTML='<p style="color:var(--text-muted);padding:1rem">Sem dados ainda.</p>';return;}
            ...
            dados.forEach(function(d){
              ...
              html+='<div class="dash-bar-col" title="'+d.data+': '+d.total+' views"><div class="dash-bar-wrap"><div class="dash-bar" style="height:'+pct+'%%"></div></div><span class="dash-bar-label">'+d.data.slice(5)+'</span></div>';
            });
            html+='</div>';
          ...
          for i, n in ipairs(self.top_semana) do
            li(function()
              ...
              div({ class = "top-info" }, function()
                a({ href = "/noticias/" .. n.id, target = "_blank", class = "top-titulo" }, n.titulo)
                span({ class = "top-views" }, "👁 " .. tostring(n.views_periodo) .. " views")
              end)
            end)
          end
        end)
      else
        p({ class = "sem-dados" }, "Sem dados ainda.")
      end
    end)
  end)
 
  -- Notícias (com botão SEO e histórico)
  div({ class = "admin-section shadow-card" }, function()
    ...
    if self.noticias and #self.noticias > 0 then
      element("table", { class = "admin-table" }, function()
        thead(function()
          tr(function()
            th("ID"); th("Título"); th("Cat."); th("Dest."); th("Views"); th("Data"); th("Ações")
          end)
        end)
        tbody(function()
          for _, n in ipairs(self.noticias) do
            tr(function()
              ...
              td({ class = "acoes-col" }, function()
                ...
                a({ href = "/admin/noticias/" .. n.id .. "/seo",       class = "btn-seo"    }, "🔍")
                a({ href = "/admin/noticias/" .. n.id .. "/historico", class = "btn-editar" }, "🕓")
  ...
  div({ class = "admin-section shadow-card" }, function()
    div({ class = "section-header" }, function()
      ...
      a({ href = "/admin/lancamentos/novo", class = "btn-novo",
          style = "margin-left:.5rem" }, "+ Lançamento")
    end)
  ...
  div({ id = "comentarios", class = "admin-section shadow-card" }, function()
    div({ class = "section-header" }, function()
      ...
      div({ class = "coment-filtros" }, function()
        a({ href  = "/admin#comentarios",
            class = "filtro-btn" .. (not self.filtro_pendentes and " filtro-ativo" or "") }, "Todos")
        a({ href  = "/admin?pendentes=1#comentarios",
          ...
          if (self.pendentes_count or 0) > 0 then
            span({ class = "badge-notif", style = "margin-left:.4rem" },
    ...
    if self.comentarios and #self.comentarios > 0 then
      element("table", { class = "admin-table" }, function()
        thead(function()
          tr(function()
            th("St."); th("Autor"); th("Notícia"); th("Comentário"); th("Data"); th("Ações")
          end)
        end)
        tbody(function()
          for _, c in ipairs(self.comentarios) do
            tr(function()
              td(c.aprovado == 1 and "✅" or "⏳")
              td(c.autor)
